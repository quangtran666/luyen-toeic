...
export function ExplanationGrid({ explanations }: ExplanationGridProps) {
	...
	return (
		<div
			...
			role="list"
			aria-label={`${explanations.length} câu hỏi được giải thích`}
		>
			{explanations.map((explanation, index) => (
				<div key={`${explanation?.questionNumber || index}`} role="listitem">
					<QuestionCard explanation={explanation} index={index} />
				</div>
			))}
		</div>
	);
